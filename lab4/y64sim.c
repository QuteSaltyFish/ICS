...
bool_t cond_doit(cc_t cc, cond_t cond) {
    ...
    switch (cond) {
        case C_YES:
            ...
            break;
        case C_LE:
            if (GET_ZF(cc) == 1 || GET_SF(cc) != GET_OF(cc))
                doit = TRUE;
            break;
        case C_L:
        ...
        case C_GE:
            ...
            break;
        case C_G:
            if (GET_ZF(cc) == 0 && GET_SF(cc) == GET_OF(cc))
                doit = TRUE;
            break;
        default:
            break;
...
stat_t nexti(y64sim_t *sim) {
    ...
    switch (icode) {
        ...
        case I_NOP: /* 1:0 */
            sim->pc = next_pc;
            break;
        case I_RRMOVQ: /* 2:x reg1:reg2 */
            // Fetch
            valP = next_pc;
            if (!get_byte_val(sim->m, valP++, &operands)) {
                err_print("PC = 0x%lx, Invalid instruction address", valP - 1);
                return STAT_ADR;
            }
            rA = GET_REGA(operands);
            rB = GET_REGB(operands);
            // check if the reg exists
            if (!NORM_REG(rA)) {
                err_print("Can't open binary file '%s'", "rA");
                return STAT_INS;
            }
            if (!NORM_REG(rB)) {
                err_print("Can't open binary file '%s'", "rB");
                return STAT_INS;
            }
            valP = valP + 0;
        ...
        case I_IRMOVQ: /* 3:0 F:regB imm */
            ...
            if (!get_long_val(sim->m, valP, &valC)) {
                err_print("PC = 0x%lx, Invalid instruction address", valP);
                return STAT_ADR;
            }
            valP = valP + 8;
        ...
        case I_RMMOVQ: /* 4:0 regA:regB imm */
            ...
            // Fetch
            if (!get_byte_val(sim->m, valP++, &operands)) {
                err_print("Invalid Memory address: 0x%lx", valP - 1);
                return STAT_ADR;
            }
            rA = GET_REGA(operands);
            rB = GET_REGB(operands);
            ...
            if (!get_long_val(sim->m, valP, &valC)) {
                err_print("Invalid Memory address: 0x%lx", valP);
                return STAT_ADR;
            }
            valP = valP + 8;
            ...
            // check if the reg exists
            if (!NORM_REG(rA)) {
                err_print("RMMOVQ '%s' not none", "rA");
                return STAT_INS;
            }
            if (!NORM_REG(rB)) {
                err_print("RMMOVQ '%s' not norm", "rB");
                return STAT_INS;
            }
            valA = get_reg_val(sim->r, rA);
            ...
            if (!set_long_val(sim->m, valE, valA)) {
                err_print("Invalid Memory address: 0x%lx", valE);
                return STAT_INS;
            }
            sim->pc = valP;
            break;
        ...
        case I_MRMOVQ: /* 5:0 regB:regA imm */
            ...
            if (!get_long_val(sim->m, valP, &valC)) {
                err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, valP);
                return STAT_ADR;
            }
            valP = valP + 8;
        ...
        case I_JMP: /* 7:x imm */
            ...
            valP = valP + 8;
            cnd = cond_doit(sim->cc, ifun);
            // printf("%d 0x%lx \t", cnd, valC);
            sim->pc = cnd ? valC : valP;
            break;

        case I_CALL: /* 8:x imm */
            valP = next_pc;
            if (!get_long_val(sim->m, valP, &valC)) {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, valP);
                return STAT_ADR;
            }
            valP = valP + 8;
            ...
            if (cnd) {
                ...
                if (!set_long_val(sim->m, valE, valP)) {
                    // set_reg_val(sim->r, REG_RSP, valE);
        ...
        case I_RET: /* 9:0 */
            ...
            if (!get_long_val(sim->m, valA, &valM)) {
                err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, valA);
                return STAT_INS;
            }
